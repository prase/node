                ...
                for (const attr of node.attrs || []) {
                  ...
                  i.e. you must write:  ${attr.name}$="${attr.value}"`),
                    ...
                    if (!found) {
                        ...
                        warnings.push({
                            code: this.code,
                            message: util_1.stripWhitespace(`${node.tagName} elements do not have ${bindingType} ` +
...
exports.SetUnknownAttribute = SetUnknownAttribute;
function contains(position, range) {
...
function closestOption(name, isAttribute, element) {
    ...
    const closestAttribute = minBy(attributeOptions, (option) => levenshtein.get(name, option));
    const closestProperty = minBy(propertyOptions, (option) => levenshtein.get(name, option));
